...
function smooth(){
 const go=e=>{
   const t=e.currentTarget,href=t.dataset.target||t.getAttribute("href");
   if(!href||!href.startsWith("#"))return;
   const trg=$(href); if(!trg)return;
   e.preventDefault(); trg.scrollIntoView({behavior:"smooth"});
 };
 $$('a[href^="#"],[data-target^="#"],#hero-cta').forEach(a=>a.addEventListener("click",go));
}
...
function decision(){
 ...
 let shown=false;
 const onScr=()=>{ if(!shown&&scrollY>180){el.classList.add("show");shown=true;removeEventListener("scroll",onScr);} };
 addEventListener("scroll",onScr,{passive:true});

 const span=$("#micro-proof");
 const msgs=["keine Vorbereitung notwendig","dauert nur 2 Minuten","du entscheidest – nichts wird verkauft"];
 if(span){
   let i=0;
...
/* ---------------- Sticky CTA ---------------- */
function sticky(){
 const cta=$("#sticky-cta"),hero=$("#hero"),quiz=$("#quiz");
 if(!cta||!hero)return;
 ...
 if(quiz){
  const ioQ=new IntersectionObserver(([e])=>{if(e.isIntersecting)show(false)}, {threshold:.25});
  ioQ.observe(quiz);
 }
...
function feed(id,items){
 ...
 const rot=setInterval(()=>{if(!paused){i=(i+1)%items.length;box.textContent=items[i]}},3500);
...
function bars(){
 const io=new IntersectionObserver(es=>{
   es.forEach(({isIntersecting,target})=>{
     ...
     const v=parseInt(target.dataset.value||target.dataset.percentage||"0",10);
     const span=$("span",target); if(span) span.style.width=Math.min(100,Math.max(0,v))+"%";
   });
 },{threshold:.35});
...
let persona="";
function personas(){
 const out=$("#persona-copy"),cards=$$(".persona-card");
 const set=c=>{
  cards.forEach(x=>x.classList.remove("is-active"));
  c.classList.add("is-active");
  persona=c.dataset.key||"";
  out.textContent=(c.dataset.headline||"")+" "+(c.dataset.subheadline||"");
 };
 cards.forEach(c=>{
  c.addEventListener("click",()=>set(c));
  c.addEventListener("keydown",e=>{if(e.key===" "||e.key==="Enter"){e.preventDefault();set(c)}})
 });
}

/* ---------------- Quiz ---------------- */
function quiz(){
 const steps=$$("#quiz-steps .quiz-step"); if(!steps.length)return;

 const bar=$("#quiz-progress-bar"),stepTxt=$("#quiz-step-text"),tick=$("#progress-tick"),soft=$("#soft-permission");
 const res=$("#quiz-result"),note=$("#quiz-summary-text"),badge=$("#persona-badge");

 const ans=[], labels=["Los geht’s","✓ 1 von 3 – gut dabei","✓ 2 von 3 – fast geschafft","✓ 3 von 3"];
 let i=0;

 const upd=()=>{
   const pct=Math.round((i/steps.length)*100);
   bar.style.width=pct+"%"; bar.ariaValueNow=pct;
   stepTxt.textContent=Math.min(i+1,steps.length);
   tick.textContent=labels[Math.min(i,labels.length-1)];
 };
 ...
 const txt=color=>{
   ...
   const arr=base[color],pick=arr[Math.random()*arr.length|0];
   return pick+(per[persona]?.[color]||"");
 };

 const paint=c=>{
  $$("[data-lamp]").forEach(l=>l.classList.remove("ampel--active"));
  const t=$(`[data-lamp="${c}"]`); if(t)t.classList.add("ampel--active");
 };

 const slots=()=>{
  const box=$("#slot-container"); box.innerHTML="";
 ...
 const show=()=>{
  ...
  const c=ampel();
  paint(c);
  note.textContent=txt(c);
  slots();
  badge.hidden=!persona; if(!badge.hidden) badge.textContent=({familie:"Familie",angestellt:"Angestellt",selbststaendig:"Selbstständig"})[persona];
  res.hidden=false;
  bar.style.width="100%"; bar.ariaValueNow="100";
  $("#progress-tick").textContent="✓ 3 von 3";
 };

 steps.forEach((st,idx)=>{
  st.addEventListener("click",e=>{
    const b=e.target.closest(".answer"); if(!b)return;
    const v=b.dataset.value; ans[idx]=v;
    if(soft)soft.style.display="none";
    auto(idx,v);
    if(idx<steps.length-1){
      steps[idx].hidden=true; steps[idx+1].hidden=false; i=idx+1; upd();
    }else show();
  });
 });

 $("#quiz-again")?.addEventListener("click",()=>{
  ans.length=0; persona=""; badge.hidden=true;
  steps.forEach((s,x)=>s.hidden=x!==0);
  $("#persona-copy").textContent="";
  $$(".persona-card").forEach(c=>c.classList.remove("is-active"));
  if(soft)soft.style.display="";
  i=0; upd(); res.hidden=true;
 });
...
function bottom(){
 ...
 ].filter(x=>document.querySelector(x[0])&&x[1]);

 const io=new IntersectionObserver(es=>{
  es.forEach(e=>{
   if(e.isIntersecting){
    sec.forEach(s=>s[1].classList.remove("is-active"));
    const m=sec.find(s=>"#"+e.target.id===s[0]);
    m&&m[1].classList.add("is-active");
   }
  });
 },{threshold:.5});

 sec.forEach(s=>io.observe($(s[0])));
}

/* ---------------- Year ---------------- */
function year(){ $$("[data-year]").forEach(n=>n.textContent=new Date().getFullYear()); }

/* ---------------- Init ---------------- */
document.addEventListener("DOMContentLoaded",()=>{
 loader();smooth();reveal();decision();sticky();
 feeds();bars();personas();quiz();bottom();year();
});
