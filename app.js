// ==========================================================================
// assets/app.js — Production-grade Calm-UX engine (2026)
...
(() => {
  ...
  const toast = (msg) => {
    ...
    t.textContent=msg;
    // why: Inline styles → keine Abhängigkeit von CSS beim Fehlerfall
  ...
  function initDecisionAnchor() {
    ...
    if (!bar) return;
    const onScroll = () => {
      const show = (window.scrollY || document.documentElement.scrollTop || 0) > 180;
  ...
  function generateSummary(answers, pct, state) {
    ...
    // FIX: Template-String statt nackter ${...}
  ...
  function generateTimeSlots(n = 5) {
    ...
    // window: 09:00–18:00 every 60 min
  ...
  function initReferralEngine() {
    ...
    // FIX: korrekte Query-Selections (die kaputten Artefakte entfernt)
