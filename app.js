// /app.js
// why: Entfernt noscript-Klasse früh & setzt aktuelle Jahreszahl
...
(() => {
  ...
  if ('IntersectionObserver' in window) {
    const io = new IntersectionObserver(
      ents => ents.forEach(e => { if (e.isIntersecting) { e.target.classList.add('visible'); io.unobserve(e.target); } }),
...
/* Sticky CTA – früh triggern, nur Mobile/Tablet */
(() => {
...
(() => {
  ...
  const set = (on) => {
    ...
    if (btn) btn.textContent = on ? '✅ Kurzmodus aktiv' : '🔍 Kurzmodus';
  ...
  window.setKurzmodus = set; // why: gewollt global für evtl. spätere Hooks
})();
...
(() => {
  ...
  if (btn && box) btn.addEventListener('click', () => { box.style.display = box.style.display === 'none' ? 'grid' : 'none'; });
})();
...
(() => {
  ...
  function randomId(len = 10) {
    ...
    if (crypto.getRandomValues) crypto.getRandomValues(u); else for (let i = 0; i < len; i++) u[i] = Math.random() * 256;
    return Array.from(u, x => a[x % a.length]).join('');
  }
  function getRefId() {
    try {
      return localStorage.getItem(KEY_REFID) || (localStorage.setItem(KEY_REFID, randomId()), localStorage.getItem(KEY_REFID));
  ...
  function pick(seg) {
    const sets = variants(); const pool = (sets[seg] || sets.neutral || []);
    if (!pool.length) return '';
    const i = Math.floor(Math.random() * pool.length);
    return pool[i];
  }
  function ensureURL(text) { return /\bhttps?:\/\/\S+/i.test(text) ? text : (text.trim() + ' ' + buildURL()); }
  function setTextFrom(seg) {
    const tmpl = pick(seg || detectSeg()); if (!tmpl) return;
    area.value = tmpl.replace(/\{\{URL\}\}/g, buildURL()); updateLinks();
  }
  ...
  function updateLinks() {
    ...
    if (preview) preview.textContent = t.length <= 190 ? t : t.substring(0, t.lastIndexOf(' ', 190)) + '…';
  }

  if (area && !area.value) { setTextFrom('neutral'); }
  updateLinks();
  ...
  document.getElementById('readyMsg')?.addEventListener('click', () => {
    ...
    if (base) toast.style.cssText = base.style.cssText;
